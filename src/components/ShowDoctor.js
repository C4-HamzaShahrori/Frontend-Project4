...
const [image,setImage]=useState("")
  ...
  const getDoctorById = async () => {
    try {
      const result = await axios.get(`https://find-doctorr.herokuapp.com/doctors/${id}`);
  ...
  const addComment = async () => {
    try {
      await axios.post(
        `https://find-doctorr.herokuapp.com/comment/${id}`,
        { comment },
        {
          headers: {
  ...
  const getUserById = async () => {
    try {
      const result = await axios.get(`https://find-doctorr.herokuapp.com/users/${ localStorage.getItem("UserId")}`);
